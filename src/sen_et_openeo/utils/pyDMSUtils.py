...
def saveImg(data, geotransform, proj, outPath, noDataValue=None, fieldNames=[]):
    ...
    if len(shape) > 2:
        ds = memDriver.Create(
            "MEM", shape[1], shape[0], shape[2], gdal.GDT_Float32)
    ...
    if outPath == "MEM":
        ...
        if ext.lower() == ".nc":
            ...
            fileFormat = "COG"
            driverOpt = ['COMPRESS=DEFLATE',
                         'PREDICTOR=YES', 'BIGTIFF=IF_SAFER']
...
def reprojectSubsetLowResScene(highResScene, lowResScene,
    ...
    proj_HR, gt_HR, xsize_HR, ysize_HR, extent = getRasterInfo(highResScene)[
        0:5]

    # Reproject low res scene to high res scene's projection
    # to get the original pixel size in the new projection
    ...
    UL = pix2point(point2pix([extent[0], extent[3]],
                   gt_LR, upperBound=False), gt_LR)
    BR = pix2point(point2pix([extent[2], extent[1]],
                   gt_LR, upperBound=True), gt_LR)
...
def resampleHighResToLowRes(highResScene, lowResScene,
                            highresbandnames=None):
    # Resample high res scene to low res pixel while extracting homogeneity
    # statistics. It is assumed that both scenes have the same projection
    # and extent.
    ...
    for band in range(bands_HR):
        ...
        if highresbandnames is not None:
            # get band name and associated metadata
            scale, dtype, nodata = get_band_metadata(highresbandnames[band])
            # apply scaling and nodata value
            bandData_HR = bandData_HR.astype(np.float32)
            bandData_HR[bandData_HR == nodata] = np.nan
            if scale is not None:
                bandData_HR = bandData_HR / scale
        # compute the statistics
        aggregatedMean[:, :, band], aggregatedStd[:, :, band] =\
            _resampleHighResToLowRes(bandData_HR, ySize_LR,
                                     yRes_LR, yRes_HR,
                                     xSize_LR, xRes_LR,
                                     xRes_HR, gt_HR, gt_LR)
    if close:
...
@njit
def _resampleHighResToLowRes(bandData_HR, ySize_LR, yRes_LR,
                             yRes_HR, xSize_LR, xRes_LR, xRes_HR,
                             gt_HR, gt_LR):
    ...
    for yPix_LR in range(ySize_LR):
        ...
        yPix_HR_max = int(
            round(max(0, gt_HR[3] - (yPos_LR_min + yRes_LR)) / yRes_HR))
        for xPix_LR in range(xSize_LR):
            xPos_LR_min = gt_LR[0] + xPix_LR*xRes_LR
            xPix_HR_min = int(round(max(0, xPos_LR_min - gt_HR[0]) / xRes_HR))
            xPix_HR_max = int(
                round(max(0, xPos_LR_min + xRes_LR - gt_HR[0]) / xRes_HR))
            aggregatedMean[yPix_LR, xPix_LR] =\
                np.nanmean(
                    bandData_HR[yPix_HR_min:yPix_HR_max,
                                xPix_HR_min:xPix_HR_max])
