...
from sen_et_openeo.era5 import (
    ERA5Collection,
...
def _month_range(temporal_extent: Sequence[str]) -> List[str]:
    """Return inclusive YYYY-MM list for the requested temporal extent."""
    start = pd.to_datetime(temporal_extent[0]).to_period("M")
    end = pd.to_datetime(temporal_extent[1]).to_period("M")
    months = []
    cur = start
    while cur <= end:
...
def _extract_date_from_biopar_filename(path_obj: Path) -> Optional[pd.Timestamp]:
    """Extract date from BIOPAR filename like BIOPAR_FAPAR_2025-07-01Z.tif."""
    m = re.search(r"(\d{4}-\d{2}-\d{2})Z", path_obj.name)
    if not m:
        return None
    try:
...
def _log_biopar_coverage_sanity(
    download_results: Dict[str, Dict[str, Any]], temporal_extent: Sequence[str]
) -> None:
    """Log BIOPAR per-month coverage and warn on mismatches across variables."""
    vars_ = SenETDownload.BIOPAR_VARIABLES
    expected_months = _month_range(temporal_extent)

    logger.info("** BIOPAR coverage sanity check")
    logger.info(
        f"   Expected month window: {expected_months[0]} -> {expected_months[-1]}"
    )

    month_counts_by_var: Dict[str, Counter] = {}
    totals_by_var: Dict[str, int] = {}
    for var in vars_:
        key = f"BIOPAR_{var}"
        files = download_results.get(key, {}).get("output_files", [])
        totals_by_var[var] = len(files)

        months = []
        for f in files:
            dt = _extract_date_from_biopar_filename(Path(f))
            if dt is not None:
                months.append(dt.strftime("%Y-%m"))

        month_counts = Counter(months)
        month_counts_by_var[var] = month_counts
        covered_months = sorted(month_counts.keys())
        missing_months = [m for m in expected_months if m not in month_counts]

        logger.info(
            f"   {var}: {totals_by_var[var]} file(s), "
            f"covered months={covered_months if covered_months else []}"
        )
        if missing_months:
            logger.warning(f"   {var}: missing requested month(s): {missing_months}")
        logger.info(
            f"   {var} monthly counts: "
            f"{dict(sorted(month_counts.items())) if month_counts else {}}"
        )

    max_total = max(totals_by_var.values()) if totals_by_var else 0
    mismatch = {v: c for v, c in totals_by_var.items() if c != max_total}
    if mismatch:
        logger.warning(
            f"BIOPAR file-count mismatch across variables: {totals_by_var}. "
            "This indicates incomplete chunk completion and requires re-download."
        )

    # Month-by-month cross-variable consistency check
    union_months = sorted({m for d in month_counts_by_var.values() for m in d})
    for month in union_months:
        vals = {v: month_counts_by_var[v].get(month, 0) for v in vars_}
        if len(set(vals.values())) > 1:
            logger.warning(f"BIOPAR monthly mismatch in {month}: {vals}")


def get_era5_data(era5_tiled_folder, tile, temporal_extent):

    timestamps = pd.date_range(
        pd.to_datetime(temporal_extent[0]),
    ...
    era5col = era5col.check_tiledates(tile, timestamps, era5_tiled_folder)

    df = era5col.df.copy()
    df["day"] = [pd.to_datetime(d).strftime("%Y-%m-%d") for d in df["date"].values]
...
def compute_lst_ta(lst_file, time, elev_file, time_zone, era5col, outfile):
    ...
    with rasterio.open(outfile, "w", **output_profile) as dst:
        ...
        dst.units = ["K"]
        # Generate overviews
        dst.build_overviews((4, 8, 16), Resampling.average)
...
def main(
    ...
    download_results = data_download.download(
        output_dir,
    ...
    if compute_et_tseb:
        _log_biopar_coverage_sanity(download_results, temporal_extent)
    ...
    if corr_parameters is not None:
        ...
        lst_dict = data_download.lst_correction(output_dir, corr_parameters)
    else:
    ...
    if generate_lstm_like:
        ...
        for t in timestamps:
            ...
            if not lstm_ta_file.exists():
                compute_lst_ta(lstm_out, t, elev_file, time_zone, era5col, lstm_ta_file)
            if not lstm_et_file.exists():
                ...
                elev_30m = resample_to_match(
                    elev_file, lstm_out, inp30 / "elev.tif", Resampling.bilinear
                )
                ...
                for var, date_paths in biopar_dict.items():
                    ...
                    for bdate, bpath in date_paths.items():
                        bdst = inp30 / (f'{var}_{bdate.strftime("%Y%m%d")}.tif')
...
if __name__ == "__main__":
    ...
    output_dir = Path("/path/to/output/2025")
    era5_tiled_folder = Path("/path/to/ERA5")
    time_zone = 0
